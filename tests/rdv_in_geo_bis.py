...
def scenario(eT, wl, wscl, fig_display=True, fig_save=True):
    ...
    sma_C = DU + 30e6 # semi-major axis, m
    ecc_C = eT # eccentricity - can't be 0 nor 1 due to singularities in the RQ-Law formulation
    inc_C = 0.01 # inclination, rad - can't be pi due to singularities in the MEE (tan(inc/2) in h and k)
    raan_C = 0 # RAAN, rad
    ...
    inc_T = 0.01 # inclination, rad - can't be pi due to singularities in the MEE (tan(inc/2) in h and k)
    raan_T = 0 # RAAN, rad
    ...
    woe1 = [2, 50, 50, 0, 0] # Lyapunov function weights, in terms of MEE with sma ([sma, f, g, h, k])
...
if __name__ == "__main__":
    ...
    exitcodes = []
    for j in range(8):
        end_mass, mass_delta, tof, exitcode = scenario(eT[j], Wl[j], Wscl[j], fig_display=False, fig_save=True)
