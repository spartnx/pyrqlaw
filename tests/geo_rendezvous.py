...
def scenario(ta0, fig_display=True, fig_save=True):
    ...
    inc_T = np.radians(5) # inclination, rad - can't be pi due to singularities in the MEE (tan(inc/2) in h and k)
    raan_T = 0 # RAAN, rad
    aop_T = 0 # argument of periapse, rad
    ta_T = np.radians(35) # true anomaly, rad
    ...
    woe1 = [14.83441486, 14.30670119, 45.87183334, 22.46458241, 15.22342023] # Lyapunov function weights, in terms of MEE with sma ([sma, f, g, h, k])
    ...
    if fig_save:
        fig11.savefig(f"../plots/geo_rendezvous/Elements history (stages 1, 2) - ta0 {int(np.degrees(ta0))}")
        fig21.savefig(f"../plots/geo_rendezvous/Trajectory (stages 1, 2) - ta0 {int(np.degrees(ta0))}")
        fig31.savefig(f"../plots/geo_rendezvous/Controls history (stages 1, 2) - ta0 {int(np.degrees(ta0))}")
        fig12.savefig(f"../plots/geo_rendezvous/Elements history (stage 1) - ta0 {int(np.degrees(ta0))}")
        fig22.savefig(f"../plots/geo_rendezvous/Trajectory (stage 1) - ta0 {int(np.degrees(ta0))}")
        fig32.savefig(f"../plots/geo_rendezvous/Controls history (stage 1) - ta0 {int(np.degrees(ta0))}")
        fig13.savefig(f"../plots/geo_rendezvous/Elements history (stage 2) - ta0 {int(np.degrees(ta0))}")
        fig23.savefig(f"../plots/geo_rendezvous/Trajectory (stage 2) - ta0 {int(np.degrees(ta0))}")
        fig33.savefig(f"../plots/geo_rendezvous/Controls history (stage 2) - ta0 {int(np.degrees(ta0))}")
    if fig_display:
        plt.show()
        #############################
